...
The plugin must be exact, conservative, and secure. Do not expose provider credentials to the browser.
...
- Inline block rewrite actions are injected through the `editor.BlockEdit` filter.
- Inline block rewrite actions use `BlockControls` and `ToolbarDropdownMenu`.
...
- Never send provider credentials to JavaScript through `wp_localize_script()` or inline scripts.
...
- Password fields must render blank and preserve existing values when submitted blank.
...
- Inline block rewrite actions must use the same authenticated AI endpoint as the sidebar.
...
- Do not log secrets.
- Do not include provider credentials in exception messages, REST responses, browser data, screenshots, tests, or documentation examples.
...
- Authenticated callers need the correct Tools AI scope for the endpoint.
...
- Keep browser-side code free of provider credentials.
...
- Inline block toolbar actions must use `BlockControls` and should be injected through `editor.BlockEdit`.
- Inline block toolbar actions should not duplicate provider configuration UI; they use saved defaults.
- Inline block toolbar actions must replace only the selected block unless a future feature explicitly supports multi-block operations.
...
## Inline rewrite toolbar behavior

The inline toolbar should remain a quick action surface, similar to other editor AI integrations.
...
- The selected block text becomes `custom_text`.
- The action text becomes `prompt`.
- The response must be treated as Markdown and converted to blocks.
- Replace the selected block with generated blocks.
- Show a clear editor error when a block has no readable text.
- Keep one-click actions deterministic and short.
- Put complex free-form instructions behind the `Ask AI Assistant` modal.
...
- Activate plugin.
- Save settings with a new OpenAI provider value.
- Save settings again with OpenAI provider value blank and confirm the old value still works.
- Save settings with a new Tools provider value.
- Save settings again with Tools provider value blank and confirm the old value still works.
...
- Confirm the selected block toolbar includes the Tornevall AI rewrite menu.
...
- Use the inline toolbar to rephrase, shorten, expand, summarize, and run a custom Ask AI instruction.
- Confirm inline toolbar rewrites replace only the selected block.
...
- Do not put provider credentials into JavaScript.
