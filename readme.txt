...
* Configurable default persona and provider.
...
* Per-block actions such as rephrase, simplify, summarize, expand, shorten, tone change, and translate.
...
6. Open the block editor and use the Tornevall AI sidebar, inline block toolbar menu, or Tornevall AI Assistant block.

== Frequently Asked Questions ==

= Are provider credentials exposed in the browser? =

No. They are stored in WordPress options and used by PHP when the local REST endpoint forwards the request.
...
Yes. Set the default provider to OpenAI direct and configure an OpenAI key and model.

== Changelog ==

= 0.3.0 =
* Added inline selected-block rewrite menu.
* Added per-block rewrite actions and custom instruction modal.
* Added shared editor AI request helper.
